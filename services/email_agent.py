...
class EmailAgentService:
    ...
    @property
    def router(self):
        """Lazy load EmailRouter to avoid circular imports."""
    ...
    def _get_action_registry(self):
        """Lazy load ACTION_REGISTRY to avoid circular imports."""
    ...
    def _get_action(self, action_name: str):
        ...
        if action_name not in self._actions:
            action_class = self._get_action_registry().get(action_name)
            if action_class:
    ...
    def _format_response(self, action_name: str, decision: Dict[str, Any],
        ...
        if result.get('skipped') is True:
            ...
            actions_list = '\n'.join(
                f"• {val['description']}"
                for key, val in get_action_descriptions().items()
                if key != 'NONE'
            )
